    ...
    public function up()
    {
        ...
        if (!$schema->hasTable('user_key_requests')) {
            $schema->create('user_key_requests', function (Blueprint $table) {
                ...
                $table->foreign('key_id', 'FK_user_keys_user_key_requests')->references('id')->on(config('database.connections.dbp_users.database') . '.user_keys')->onUpdate('cascade')->onDelete('cascade');
                $table->string('name');
                $table->string('email');
